...
function duplicateEncode(word) {
  ...
  const arrayLetter = wordLowerCase.split("");

  return arrayLetter.reduce((res, letter, index) => {
    const wordWithoutLetter = wordLowerCase.slice(0, index) + wordLowerCase.slice(index + 1);

    res += wordWithoutLetter.toLowerCase().includes(letter.toLowerCase()) ? ")" : "(";
