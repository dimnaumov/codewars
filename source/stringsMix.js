...
// appears as many times as its maximum if this maximum is strictly greater
// than 1; these letters will be prefixed by the number of the string where
// they appear with their maximum value and :. If the maximum is in s1 as well
...
function mix(s1, s2) {
  function getObjectChars(str, prefix) {
    const lowercaseLetters = str.match(/[a-z]+/g).join('')
    const chars = {};

    for (const char of lowercaseLetters) {
      chars[char] = (chars[char] || '') + char;
    }

    return Object.fromEntries(
      Object.entries(chars)
        .filter(([_key, value]) => value.length > 1)
        .map(([key, value]) => [key, `${prefix}:${value}`])
    );
  }

  const chars1 = getObjectChars(s1, 1);
  const chars2 = getObjectChars(s2, 2);
  ...
  const result = chars.reduce((acc, char) => {
    const entry1 = chars1[char];
    const entry2 = chars2[char];
    ...
    if (entry1 && entry2) {
      if (entry1.length > entry2.length) {
        ...
        acc.push(`=:${entry1.split(':')[1]}`);
      }
    } else {
      acc.push(entry1 || entry2);
    }
  ...
  result.sort((a, b) => {
    const [prefixA, charsA] = a.split(':');
    const [prefixB, charsB] = b.split(':');
    ...
    if (prefixA === prefixB) {
      return charsA.localeCompare(charsB);
    }
