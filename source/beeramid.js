...
const beeramid = function(bonus, price) {
  const totalBeerCount = Math.floor(bonus / price);

  if (!Number.isInteger(totalBeerCount) || totalBeerCount <= 0) {
    return 0;
  }

  const levelCalc = (beerCount, level = 0) => {
    const needBeerCountForNextLevel = Math.pow(level + 1, 2);

    if(needBeerCountForNextLevel > beerCount) {
      return level;
    }

    return levelCalc(beerCount - needBeerCountForNextLevel, level + 1);
  }

  return levelCalc(totalBeerCount);
}
