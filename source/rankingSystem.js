...
//    be worth 3 points
//  - Completing an activity that is ranked one ranking lower than the user's
...
// uses a pure Math based solution that gives consistent results no matter what
// order a set of ranked activities are completed at.

class User {
  #rankPointsCount = 100;
  #rankRange = [-8, -7, -6, -5, -4, -3, -2, -1, 1, 2, 3, 4, 5, 6, 7, 8];
  #rankThresholdIncStop = 2;
  #actionPointsCurrentRank = 3;
  #actionPointsLessRank = 1;
  ...
  get rank() {
    return this.#rankRange[this.rankIndex];
  }

  incProgress(rankAction) {
    if (!this.#rankRange.includes(rankAction)) {
      throw new Error('Error rating action');
    }

    const indexRankAction = this.#rankRange.findIndex((rank) => rank === rankAction);

    const diff = indexRankAction - this.rankIndex;

    if (diff === 0) {
      this.progress += this.#actionPointsCurrentRank;
    } else if (diff > 0) {
      this.progress += 10 * diff * diff;
    } else if (diff > -this.#rankThresholdIncStop) {
      this.progress += this.#actionPointsLessRank;
    }

    const rankUpIndex = Math.floor(this.progress / this.#rankPointsCount);
    const rankIndexMax = this.#rankRange.length - 1;
    
    this.rankIndex = Math.min(this.rankIndex + rankUpIndex, rankIndexMax);

    this.progress = this.rankIndex === rankIndexMax
      ? 0
      : this.progress % this.#rankPointsCount;
  }
}
